"""
Orquestador de la Fase 2 (automatización) para Diplomados.

Qué hace, en orden:
  1. Se conecta a Google Drive (cuenta de servicio) y lista los archivos de la carpeta
     DATOS/diplomados/ (DRIVE_FOLDER_ID).
  2. Compara contra diplomas.archivo_procesado: si un archivo (por id + md5) ya se
     procesó antes, lo ignora. Si es nuevo o cambió su contenido, sigue.
  3. Para cada archivo nuevo: lo descarga, corre normalizar_diplomados.normalizar()
     y generar_sql_carga.generar() (el MISMO código que ya está probado en
     pipeline_diplomados/, sin duplicar lógica).
  4. Si hay advertencias "critico" (un dato sin el tipo esperado) -> NO carga nada a la
     ...
     Esto es el "ambiente intermedio de revisión" que acordaron Sara/Angie: en vez de una
     base de staging aparte, el propio robot se frena antes de tocar diplomas si algo
     no pasa la validación de tipos.
  5. Si no hay críticas -> ejecuta 02_cargar_catalogos.sql + 03_cargar_diplomados.sql
     (UPSERT, modo acumular) contra Postgres.
  6. Manda un correo con el resumen (a NOTIFY_EMAIL_TO) y registra cada archivo en
     diplomas.archivo_procesado.
...
No toca core, academic_workload, ni ningún otro esquema. Solo diplomas.
"""
...
# pipeline_diplomados/ (carpeta padre) tiene normalizar_diplomados.py y generar_sql_carga.py;
# se reutilizan tal cual, sin copiar ni reescribir su lógica.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
from normalizar_diplomados import normalizar  # noqa: E402
...
def conectar_db():
    # Conexión directa por IP pública (no por el Cloud SQL Auth Proxy / roles/cloudsql.client):
    # el equipo de infraestructura decidió no dar ese permiso todavía, así que el robot se
    # conecta como pgAdmin -- por IP + usuario + contraseña -- con una IP FIJA propia (creada
    # con un conector VPC + Cloud NAT) que se agregó a las "redes autorizadas" de core-database,
    # igual que las IPs de las personas del equipo. sslmode=require porque viaja por internet
    # abierto, no por el túnel privado de Google.
    return psycopg2.connect(
        host=os.environ.get("DIPLOMADOS_DB_HOST", "localhost"),
        port=os.environ.get("DIPLOMADOS_DB_PORT", "5432"),
        dbname=os.environ["DIPLOMADOS_DB_NAME"],
        user=os.environ["DIPLOMADOS_DB_USER"],
        password=os.environ["DIPLOMADOS_DB_PASSWORD"],
        sslmode="require",
    )
...
def archivos_ya_procesados(conn) -> set[tuple[str, str | None]]:
    with conn.cursor() as cur:
        cur.execute("SELECT drive_file_id, md5_checksum FROM diplomas.archivo_procesado;")
...
def registrar(conn, archivo: dict, stats: dict, estado: str, detalle: str = "") -> None:
    with conn.cursor() as cur:
        cur.execute(
            """
            INSERT INTO diplomas.archivo_procesado
                (drive_file_id, nombre_archivo, md5_checksum, ordenes_en_archivo,
                 advertencias_totales, advertencias_criticas, estado, detalle)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            """,
            (
                archivo["id"], archivo["name"], archivo.get("md5Checksum"),
                stats.get("ordenes"), stats.get("advertencias"),
...
def notificar(asunto: str, cuerpo: str) -> None:
    # NOTIFY_EMAIL_TO admite uno o varios correos separados por coma (ej.
    # "angie_vera@cun.edu.co,sara_martinezl@cun.edu.co") -- se le manda a todos a la vez.
    to_raw = os.environ.get("NOTIFY_EMAIL_TO")
    to_addrs = [addr.strip() for addr in to_raw.split(",")] if to_raw else []
    smtp_user = os.environ.get("SMTP_USER")
    smtp_password = os.environ.get("SMTP_PASSWORD")
    # Mientras no esté configurado el correo remitente (pausado a propósito, ver
    # DESPLIEGUE.md), no se intenta enviar nada -- el resumen queda en los logs de Cloud
    # Run (gcloud logging read ...) y en diplomas.archivo_procesado, sin que el job falle.
...
def procesar_archivo(drive, conn, archivo: dict) -> dict:
    with tempfile.TemporaryDirectory() as tmp_str:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute((salida / "03_cargar_diplomados.sql").read_text(encoding="utf-8"))
        conn.commit()
...
def main() -> None:
    ...
    lineas = [f"Diplomados — revisión automática de Drive ({len(nuevos)} archivo(s) nuevo(s)):", ""]
    for r in resultados:
        if r["estado"] == "cargado":
            lineas.append(
                f"✅ {r['archivo']}: {r.get('ordenes')} órdenes cargadas "
    ...
    print(cuerpo)
    notificar("Diplomados — carga automática semanal", cuerpo)
